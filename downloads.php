...
{
  var $url, $cibp, $pams, $div1, $div2, $sql, $thankYouMessage;
  function __construct() {
    ...
    $this->div1 = 'main1';
    $this->div2 = 'main2';
	$this->sql = "select * from myresources where active='Y' ";
	$this->thankYouMessage = "<span class='colorRed'>Successfully upload the resources.</span>";
  }
...
<table class="table8" align="center">
  ...
  while ($row = mysql_fetch_array($result)) {
    ...
    <tr class="<?=$bgcolor;?>" align="right">
      ...
      <td class="bdR"><form method="post" action="<?=$this->url;?>" id="t_<?=$row['id'];?>"> <!-- onchange="$(this).title('t_< ?=$row['id'];?>')" -->
...
-->          <textarea name="title" readonly="readonly" class="dlTitle" ondblclick="$(this).removeAttr('readonly').css('border-width','2px');" onchange="$(this).title('t_<?=$row['id'];?>')"><?=htmlspecialchars($row['title']);?></textarea>
      ...
      <td class="bdR"><form method="post" action="<?=$this->url;?>" id="fd_<?=$row['id'];?>" style="margin:0;padding:0;">
          <textarea name="notes" class="dlNotes" readonly="readonly" ondblclick="$(this).removeAttr('readonly').css('border-width','2px');" onchange="$(this).comment('fd_<?=$row['id'];?>')"><?=htmlspecialchars($row['comment']);?></textarea>
          <input type="hidden" name="id" value="<?=$row['id'];?>" />
        </form></td>
      <td class="bdR"><label>
...
<div>
  <form method="post" action="javascript:void();" id="form_uploader">
    ...
    <div class="marB7px">
      ...
      <label>Public</label>
      &nbsp;
      <input type="checkbox" name="location" value="M" checked="checked" />
      <label>Members</label>
    </div>
    <div>
      <label for="title">Title:&nbsp;&nbsp;(<span style="color:#FF0000">Optional. Please add/edit the title and comments from the <strong>List & Update</strong> tab.</span>)</label>
    </div>
    <div>
...
$(function() {
	$("#uploader").pluploadQueue({
		...
		multipart_params: { 'title':'','comment':'','location':'' },
		...
		init : {
		  ...
		  BeforeUpload : function(up, files) { // Fires when just before a file is uploaded. This event enables you to override settings on the uploader instance before the file is uploaded.
		  	if ($("#form_uploader input[name='location']:checked").size() == 0) {
				alert("Please assign at least 1 checkbox.");
				this.state = plupload.STOPPED;
				this.trigger("StateChanged");
				//up.stop(); return false;
				$('.plupload_wrapper a.plupload_stop').trigger('click');
			}
			var v = $("#title").val();
			if (v=='' || /^\s+$/.test(v)) {
			...
			up.settings.multipart_params.location = $('#form_uploader input[name="location"]:checked').map(function(){ return $(this).val(); }).get().join(",");
		  },
	...
	/* Never come here.
	$('#form_uploader').submit(function(e) {
		alert('Am I alive? - form_uploader');
		e.preventDefault();
	});*/
	/*$('#uploader').click(function(e) {
		alert('Am I alive - uploader?');
		e.preventDefault();
	}); */
});
</script>
<?php
  ...
  function update_resources_file($file) {
	...
	$query = "update myresource_files set size=".$size.", content='".$content."' where file = '".$file."'";
	if (! mysql_query($query, $this->cibp)) {
		die("Could not update myresource_files at " . __LINE__ . ': '.mysql_error());
	}
	$query = "update myresources set size=".$size." where file = '".$file."'";
	if (! mysql_query($query, $this->cibp)) {
		die("Could not update myresources at " . __LINE__ . ': '.mysql_error());
	}
	// echo "update successfully.";
  }
  function delete($id) {
	$query = "update myresources set active='N', updatedby='".$_SESSION['pams_user']."' where id=".$id;
    if(! mysql_query($query, $this->cibp)) {
  ...
  function get_updated_notes($id) {
	$query = "SELECT updated,updatedby,created,createdby FROM myresources where active='Y' and id=" . $id;
  ...
  function update_notes($id, $notes) {
	...
	$query = "update myresources set comment='".$notes."', updatedby='".$username."' where id=".$id;
	//echo $query; updatedby='admin', updated=now();
	mysql_query($query, $this->cibp) or die("Can't update myresources column at line ".__LINE__.': '.mysql_error());
  ...
  function update_title($id, $title) {
	$username = isset($_SESSION['pams_user']) ? mysql_real_escape_string($_SESSION['pams_user']) : '';
	$query = "update myresources set title='".$title."', updatedby='".$username."' where id=".$id;
	mysql_query($query, $this->cibp) or die("Can't update myresources column at line ".__LINE__.': '.mysql_error());
	$row = $this->get_updated_notes($id);
	$ary = array();
	$ary['id'] = $id;
...
<form action="<?=$this->url;?>" id="form1" method="post">
  ...
  <tr>
    ...
    <td align="right"><label for="type">Location:</label></td>
...
<?php
  ...
  function get_type() {
	$query = "SELECT distinct(type) FROM myresources where active='Y' order by type";
...
/* use upload.php instead.
if( isset($_FILES) && is_array($_FILES) && count($_FILES)>0 ) {
	if($uploader->process_form_deprecated()) {
		echo $uploader->thankYouMessage;
	}
	else {
		echo $uploader->error;
	}

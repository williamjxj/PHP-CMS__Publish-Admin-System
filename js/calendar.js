...
var MonthCol = new Array("01","02","03","04","05","06","07","08","09","10","11", "12")
...
function fPopCalendar(param1, param2, param3)
{
	...
	if(datas.length == tmpo.length)
	{
		...
		datas = new Array(cw.selYear, cw.selMonth, cw.selDay);
...
function fillBackDate(tDay, tMonth, tYear)
{
	...
	if(formatInputs==1)
	{
		...
		// william on May 22,2010.
		document.getElementById(elem1).value = tYear + formatSplitter + tMonth + formatSplitter + tDay;
	}
	if(formatInputs==2)
	{
		document.getElementById(elem1).value = tYear;
		document.getElementById(elem2).value = tMonth + formatSplitter + tDay;
	}
	if(formatInputs==3)
	{
		document.getElementById(elem3).value = tDay;

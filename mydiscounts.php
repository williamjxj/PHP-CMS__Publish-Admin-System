...
if(isset($_REQUEST['action'])) {
	...
	case 'add':
		$new_record = $myd->add(); // $_POST;
		echo json_encode($new_record);
		break;
	case 'edit':
		$edit_record = $myd->edit(); // $_POST;
		echo json_encode($edit_record);
		break;
	case 'delete':
		if($myd->delete($_GET['id'])) {
...
elseif(isset($_GET['add_list'])) {
	$myd->lists($_GET['add_list']);
}
elseif(isset($_GET['sort_ids'])) {
	$myd->change_sort();
}
elseif(isset($_GET['refresh'])) {
	$myd->lists($_GET['refresh']);
}
else {
	$myd->init();
}
...
class MyDiscounts
{
  ...
  function __construct() {
	...
	$this->div2 = "main2";
	$this->div3 = "main3";
	$this->sql1 = "SELECT * FROM mydiscounts where weight=".WEIGHT." and active='Y' and location='M' ";
	$this->sql2 = "SELECT * FROM mydiscounts where weight!=".WEIGHT." and active='Y' and location='M' order by weight, updated desc, created desc ";
	$this->sql3 = "SELECT * FROM mydiscounts where weight=".WEIGHT." and active='Y' and location='P' ";
	$this->sql4 = "SELECT * FROM mydiscounts where weight!=".WEIGHT." and active='Y' and location='P' order by weight, updated desc, created desc ";
...
$(function() {
  ...
  //if ($('#f_new').is(':visible')) $('#f_new').hide('fast'); else $('#f_new').show('fast');
...
/*  $(ol+'>li').each(function(index){
  ...
  $(ol+">li>div>span[class='title'],"+ol+">li>div>span a[name='edit']").bind('click', function(event) {
	if ( $.browser.msie ) { /* Ryan added: for IE */	 
		$(ol + ' > li').addClass('padT5px padB5px').removeClass('addPad');
		$(ol + ' > li div.listWrap').addClass('listWrap_off').removeClass('listWrap');
	} /* end of IE */
	event.preventDefault();
	var $div = $(event.target).closest('div');
	// $div.closest('li').removeClass(); //remove all classes inherited from <li> etc.
	var id = $div.attr('id').substr(4);
	var title = $div.children(':first').text(); // (M) the tilte
	title = jQuery.trim(title);
...
<?
  ...
  function get_current_id($id) {
	$query = "SELECT * FROM mydiscounts where active='Y' and id=" . $id;
	$res = mysql_query($query, $this->cibp);
	$row = mysql_fetch_assoc($res);
	mysql_free_result($res);
	return $row;
  ...
  function edit()
  {
	...
	$id = $_POST['id'];	
    $query = "update mydiscounts set title = '".$title."', content = '".$content."', updatedby='".$_SESSION['pams_user']."' where id=".$id;
      if(! mysql_query($query, $this->cibp)) {
        die ("Could not update mydiscount at [".__LINE__.']: '.mysql_error());
      }
	  return $this->get_current_id($id);
  }  
  function get_max_weight($location) {
	$sql = "select max(weight) from mydiscounts where active='Y' and location='".$location."'";
  ...
  function add()
  {
	...
	if(is_array($location) && count($location)>1) {
		foreach($location as $t) {
			...
			$weight = $max ? (++$max) : 100;
			$query = "insert into mydiscounts (title, content, weight, createdby, created, location)
  ...
  function delete($id) {
    // $query = "delete from mydiscounts where id=".$id; // and id!=1
	$query = "update mydiscounts set active='N', updatedby='".$_SESSION['pams_user']."' where id=".$id;
    if(! mysql_query($query, $this->cibp)) {
        die ("Could not delete mydiscounts id=".$id." at [".__LINE__."]: ".mysql_error());
    }
	return true;
  }
  function change_sort() {
	...
	//echo $_GET['sort_ids']; print_r($ary); return false;
	$count = 1;
	foreach($ary as $id) {
		$sql = "update mydiscounts set weight=".$count.", updatedby='".$_SESSION['pams_user']."' where id=" . $id;
		mysql_query($sql,  $this->cibp) or die(mysql_error());
		$count ++;
	}
  }
  function get_titleid_by_weight($location) {
      $query = "select id from mydiscounts where weight=".WEIGHT." and location='".$location."'";
      $res = mysql_query($query, $this->cibp);
  ...
  function add_deprecated() {
	...
	$content = mysql_real_escape_string(trim($_POST['content']));
	$weight = $_POST['weight'];
    $query = "insert into mydiscounts (title, content, weight, created)
      values('".$title."', '".$content."', ".$weight.", now())
      on duplicate key update
      content = '".$content . "'";
      if(! mysql_query($query, $this->cibp)) {
        die ("Could not add/update mydiscount at [".__LINE__.']: '.mysql_error());
      }
  }
}
?>
